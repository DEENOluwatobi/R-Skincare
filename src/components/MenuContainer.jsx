    ...
    <section className='w-full my-4' id='menu'>
        <div className='w-full flex flex-col items-center justify-center'>
          <p className='text-lg font-semibold capitalize text-headingColor relative before:absolute
              before:rounded-lg before:bg-gradient-to-tr from-red-300 to-red-600 before:content before:w-14 before:h-1 before:-bottom-2
              before:left-0 transition-all ease-in-out duration-100 mr-auto'>
                Our Hot Dishes
          </p>
          ...
          <div className='w-full flex items-center justify-start lg:justify-center gap-8 mt-6 py-6 overflow-x-scroll scrollbar-none'>
            {categories && categories.map(category => (
              ...
              whileTap={{ scale : 0.8 }}
              key={category.id}
              className={`group ${filter === category.urlParamName ? 'bg-red-600' : 'bg-card'} w-24 min-w-[94px] h-28 cursor-pointer rounded-lg drop-shadow-lg flex flex-col gap-3 
